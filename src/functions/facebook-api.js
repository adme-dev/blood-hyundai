  ...
  try {
    ...
    listings.ele('link', { rel: 'self', href: 'https://traralgonhyundai.com.au/cars-sales/?r=vddn' });
      ...
      jsonData.forEach(item => {
        ...
        //   const imageElement = listing.ele('image');
        //   imageElement.ele('url', item.photos[0].Url);
        //   imageElement.ele('tag', 'Exterior');  // Assuming the first image is always of the exterior
        // }
        ...
        // Assuming you want to add multiple images
        // if (item.photos && item.photos.length) {
        //   item.photos.forEach(photo => {
        //     listing.ele('image').ele('url', photo.Url);
        //   });
        // }

        if (item.photos && item.photos.length) {
          for(let i = 0; i < Math.min(item.photos.length, 20); i++) {
              listing.ele('image').ele('url', item.photos[i].Url);
          }
      }
