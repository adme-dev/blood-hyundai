...
<div v-if="filteredItems">
  ...
  <select v-model="category">
    <option v-for="(modelCategory, index) in vehiclecat" :key="index"  :value="modelCategory" class="uk-text-bold">
      {{modelCategory}}
    </option>
</select>

<select v-model="name">
  <option v-for="(modelCategory, index) in vehiclemodel" :key="index"  :value="modelCategory" class="uk-text-bold">
    {{modelCategory}}
  </option>
</select>
  ...
  <input type="text" v-model="name" placeholder="Filter By Name"/>
    ...
    <label for="vol">Price (between 0 and 1000): {{range}}</label>
    <input type="range" v-model="range" min="0" max="100000" step="10"/>
    <ul>
        <li v-for="product in filterProducts">Product Name : {{product.title}} - Price : {{product.price}} ({{product.category}})</li>
    </ul>
  ...
  <div class="uk-hidden@s">
    <div @click="show = !show" class="uk-padding uk-padding-remove-bottom uk-link uk-text-bold">
      ...
      <li :class="{ 'uk-active': (itemToShow=='-1') }">
        <a href="#"
        @click.prevent="checkedModelCat = 'All', itemToShow = '-1', show = false"
        class="uk-text-bold"
        >
        All
      </a>
      </li>
      <li v-for="(modelCategory, index) in vehiclecat" :key="index" :class="{ 'uk-active': (itemToShow==index) }" @mouseover="itemToShow = index">
        <a :href="'#'+ modelCategory" @click.prevent="checkedModelCat = modelCategory, show = false" class="uk-text-bold" uk-scroll="offset:80">
          {{modelCategory}}
        </a>
      </li>
      <li>
        <router-link to="/car-sales?condition=used&make=hyundai" class="uk-dropdown-close uk-text-bold uk-text-capitalize">Used Hyundai's</router-link>
      </li>
      </ul>
    </div>
  ...
  <ul class="model--Category uk-tab uk-margin-small uk-flex uk-flex-center uk-margin-medium-top uk-visible@s">
  ...
  <div class="uk-card uk-card-default scroll-card uk-box-shadow-hover-medium">
    ...
    <router-link
      :to="{
        ...
        params: {
          year: itemName.year.displayValue[0],
          make: itemName.make.value[0],
          model: itemName.model.value[0],
          badge: itemName.badge.value[0],
          stockid: itemName.stockid,
          id: itemName.id,
        },
      }"
      class="uk-link-reset"
    >
     ...
     <div class="uk-card-media-top uk-position-relative">
       <div
         ...
         :data-src="itemName.photos[0].Url.split(1920, 1080).join(280, 180)"
         data-uk-img
       ></div>
       <div
         v-else
...
export default {
  ...
  computed: {
    ...
    filterProducts: function(){
        return this.filterProductsByRange(this.filterProductsByName(this.filterProductsByCategory(this.vehicles)))
    },

    filteredItems: function() {
      if (this.checkedModelCat.length && this.checkedModelCat !== "All") {
        return this.vehicles.filter((vehicles) =>
          this.checkedModelCat.includes(vehicles.body.displayValue[0])
        );
      } else {
    ...
    vehiclecat: function() {
      if (this.vehicles.length > 0) return [...new Set(this.vehicles.map((i) => i.body.displayValue[0]))];
    },
  ...
  methods: {
            ...
            filterProductsByCategory: function(vehicles){
                return vehicles.filter(product => !product.body.displayValue[0].indexOf(this.category))
            },

            filterProductsByName: function(vehicles) {
                return vehicles.filter(product => !product.model.displayValue[0].indexOf(this.name))
            },

            filterProductsByRange: function(vehicles){
                return vehicles.filter(product => (product.price > 0 && product.price < this.range) ? product : '')
            }
        },
