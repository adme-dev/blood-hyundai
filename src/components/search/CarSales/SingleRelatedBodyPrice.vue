<template>
  <div v-if="filteredItems" uk-slider>
    <div class="uk-grid-small uk-margin-medium-top" uk-grid>
      <h6 class="uk-width-1-1 uk-padding uk-text-center uk-h3">
        <span class="uk-text-bold "
          ><span class="uk-text-uppercase">{{ itemsCount }} {{ body }}</span
          >'s
        </span>
        In Your Price Range: {{ (price - 3000) | currency }} -
        {{ (parseInt(price) + 3000) | currency }}
      </h6>
    </div>
    ...
    <div class="uk-container uk-container-expand uk-visible-toggle">
      <ul class="model--Category uk-tab uk-margin-small uk-flex uk-flex-center">
        <li v-for="(modelCategory, index) in vehiclecat" :key="index">
          <label class="uk-button text-blue uk-text-bold">
            <input
              class="uk-checkbox"
              type="checkbox"
              :value="modelCategory"
      ...
      <ul
        class="uk-slider-items uk-flex uk-flex-center@s uk-child-width-1-2 uk-child-width-1-3@s uk-child-width-1-4@l uk-grid-match related-slider scroll-card-wrap"
        uk-height-match="target: > li > .uk-card"
      >
        <li
          v-for="(itemName, index) in filteredItems"
          :key="index"
          v-bind:class="{
            'uk-hidden': selectedVariant(itemName.stockid),
          }"
        >
          <div
            class="uk-card uk-card-default scroll-card uk-box-shadow-hover-medium uk-inline"
         ...
         <div v-else class="uk-inline uk-width-1-1 img-height-medium uk-background-cover" data-src="/comingsoon.jpg" data-uk-img></div> -->
                ...
                <!-- <img v-if="itemName.photos" src="/comingsoon.jpg" :data-src="itemName.photos[0].Url" class="uk-width-1-1" data-uk-img>
         <img v-else src="/comingsoon.jpg" class="uk-width-1-1" data-uk-img> -->
              </div>
...
<script>
export default {
  name: "CarSalesSingleRelatedBodyPrice",
  data() {
    return {
      checkedModelCat: [],
    };
  ...
  computed: {
    vehicles() {
      return this.$store.getters.relatedVehicleBodyPrice(this.body, this.price, this.vehicle_id);
    },
    filteredItems: function() {
      if (this.checkedModelCat.length) {
        return this.vehicles.filter((vehicles) =>
          this.checkedModelCat.includes(vehicles.make.displayValue[0])
        );
      } else {
        return this.$store.getters.relatedVehicleBodyPrice(this.body, this.price, this.vehicle_id);
      }
    ...
    vehiclecat: function() {
      if (this.vehicles.length > 0)
        return [...new Set(this.vehicles.map((i) => i.make.displayValue[0]))];
    },
  },
  methods: {
    selectedVariant: function(variantId) {
      return variantId == this.vehicle_id;
      //console.log(variantId == this.vehicle_id)
    },
    hasItemPhoto(item) {
      return item.photos;
    },
    //  vehicleRemove: function(variantId) {
    //    if (this.stockid == this.vehicle_id) {
