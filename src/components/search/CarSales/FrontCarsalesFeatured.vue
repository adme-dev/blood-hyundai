...
<div v-if="filteredItems">
  ...
  <div class="model--Category uk-grid uk-flex uk-flex-center uk-margin-large-top uk-light">
    <div
      v-for="(modelCategory, index) in groupedModels"
      :key="index"
    >
      <button
        class="model-button uk-button uk-padding-small ext-blue"
        :class="{ 'model-button-active': checkedModelCat.includes(modelCategory.model.displayValue[0]) }"
        @click="toggleModelCategory(modelCategory.model.displayValue[0])"
      >
        ...
        <div class="uk-text-left uk-padding-small">
          ...
          >
            ...
            <div class="uk-width-expand uk-margin-small-left uk-margin-small-bottom">
              ...
              <div class="uk-h3 uk-margin-remove uk-text-secondary">
                {{ itemName.condition.displayValue[0] }}
                {{ itemName.year.displayValue[0] }} {{ itemName.make.displayValue[0] }}
                {{ itemName.model.displayValue[0] }}
              </div>
            ...
            <div class="uk-width-1-1 uk-flex uk-flex-middle uk-text-left">
              ...
              <div class="uk-width-expand uk-margin-small-left uk-margin-small-bottom">
                ...
                <div class="uk-text-secondary uk-text-uppercase uk-text-light">
                  {{ itemName.badge.displayValue[0] }} {{ itemName.series.displayValue[0] }}
                </div>
              </div>
            </div>
          </div>
...
export default {
  ...
  computed: {
    vehicles() {
      // Get the stock IDs from the store
      const stockIdsString = this.$store.state.site.stock[0].front_page[0]; // "320415964, 320403622"
      const stockIds = stockIdsString.split(',').map(id => id.trim()); // Convert to array of IDs

      // Filter vehicles by stock ID
      return this.$store.state.vehicles.filter((vehicle) => {
        return stockIds.includes(vehicle.stockid.toString()); // Assuming `stockid` is a property of the vehicle
      });
    },
    filteredItems: function() {
      if (this.checkedModelCat.length) {
        return this.vehicles.filter((vehicle) =>
          this.checkedModelCat.includes(vehicle.model.displayValue[0])
        );
      } else {
  ...
  methods: {
    toggleModelCategory(model) {
      const index = this.checkedModelCat.indexOf(model);
      if (index === -1) {
        // Add the model to the checkedModelCat array
        this.checkedModelCat.push(model);
      } else {
        // Remove the model from the checkedModelCat array
        this.checkedModelCat.splice(index, 1);
      }
    },
  },
};
...
.model-button {
  border: 2px solid #fff; /* Border color for buttons */
...
.model-button:hover {
  background-color: #1e87f0; /* Background color on hover */
  color: #fff; /* Text color on hover */
  transform: translateY(-2px); /* Slight lift effect on hover */
  box-shadow: 0 4px 8px rgba(30, 135, 240, 0.2); /* Shadow on hover */
}

.model-button-active {
  background-color: #1e87f0; /* Background color for active button */
  color: #fff; /* Text color for active button */
  border-color: #1e87f0; /* Border color for active button */
}

.model-button-active:hover {
  background-color: #1472c4; /* Darker background on hover for active button */
  border-color: #1472c4; /* Darker border on hover for active button */
}
