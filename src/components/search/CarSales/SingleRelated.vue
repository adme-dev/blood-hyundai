  ...
  <div v-if="vehicles" uk-slider>
    <div class="uk-grid-small uk-margin-medium-top" uk-grid>
      <h6 class="uk-width-1-1 uk-padding uk-text-center uk-h3">
        ...
        <span v-if="model.length > 1">{{model}}</span><span v-else>{{model}}</span>
      </h6>
    </div>


    <ul class="model--Category uk-tab uk-margin-small uk-flex uk-flex-center">
      <li v-for="(modelCategory, index) in vehiclecat" :key="index" class="uk-padding-remove">
      ...
      >
        <li v-for="(itemName, index) in filteredItems" :key="index">
         ...
         <div v-else class="uk-inline uk-width-1-1 img-height-medium uk-background-cover" data-src="/comingsoon.jpg" data-uk-img></div> -->
                ...
                <!-- <img v-if="itemName.photos" src="/comingsoon.jpg" :data-src="itemName.photos[0].Url" class="uk-width-1-1" uk-img>
...
export default {
  ...
  computed: {
    vehicles() {
      return this.$store.getters.relatedVehicleName(this.model, this.vehicle_id);
    },
    filteredItems: function() {
      if (this.checkedModelCat.length) {
        return this.vehicles.filter((vehicles) => this.checkedModelCat.includes(vehicles.badge.displayValue[0]));
      } else {
        return this.$store.getters.relatedVehicleName(this.model, this.vehicle_id);
      }
    },
    vehiclecat: function() {
      if (this.vehicles.length > 0) return [...new Set(this.vehicles.map((i) => i.badge.displayValue[0]))];
    },
  },
  created: function() {
    //alert(this.itemName.stockid)
  },
  methods: {
