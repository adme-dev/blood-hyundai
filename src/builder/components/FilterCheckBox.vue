<template>
  <div class="filter-variant-inner">
    ...
    <div
      v-if="option.displayName === 'Models'"
      class="checkbox-models-wrap uk-child-width-1-2 uk-grid-collapse uk-grid uk-flex uk-flex-bottom"
    >
      <div class="checkbox-models" v-for="variant in option.data" :key="variant.value">
        <el-checkbox @change="onCurrentChange()" v-model="value" :label="variant.value" :disabled="counters[variant.value] === 0">
          <div v-if="variant.thumb">
            <img class="checkbox-img uk-width-1-1 uk-display-block" :src="'' + variant.thumb + ''" />
          </div>
    ...
    <div v-else class="checkbox-variant" v-for="variant in option.data" :key="variant.value">
      <el-checkbox v-model="value" :label="variant.value" :disabled="counters[variant.value] === 0">
...
@media (min-width: 960px) {
  ...
  .checkbox-models .el-checkbox.is-checked .checkbox-img,
  .checkbox-models .el-checkbox:hover img {
    opacity: 1;
  }
}
</style>
