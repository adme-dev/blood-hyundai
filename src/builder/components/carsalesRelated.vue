<template>
  <div v-if="vehicles && vehicles.length > 0">
    ...
    <div class="uk-container uk-container-xsmall uk-text-center">
      <div class="uk-text-meta uk-text-uppercase uk-margin-large-top space33">{{ totalCount }} {{ model }}'S IN STOCK</div>
    </div>
    ...
    <ul class="model--Category uk-tab uk-margin-small uk-flex uk-flex-center uk-margin-medium-top">
      <li v-for="(modelCategory, index) in vehiclecat" :key="index">
        <label class="uk-button text-red uk-text-bold">
          <input class="uk-checkbox" type="checkbox" :value="modelCategory" v-model="checkedModelCat" />
    ...
    <div uk-slider>
      ...
      <div class="uk-container uk-container-expand">
        ...
        <ul class="uk-slider-items uk-child-width-1-2 uk-child-width-1-3@s uk-child-width-1-4@l related-slider"
          :class="[itemsCount < 6 ? 'uk-flex uk-flex-center@m ' : '']">
          <li v-for="(itemName, index) in filteredItems" :key="index">
              ...
              <router-link :to="{
                ...
                <div>
                  <div class="uk-text-left uk-padding-small">
                    <div v-if="itemName.price" class="uk-width-1-1 uk-flex uk-flex-middle uk-text-left" itemprop="offers"
                ...
                <div class="uk-card-media-top uk-position-relative">
                  <div v-if="itemName.photos.length" class="uk-inline uk-width-1-1 img-height-medium uk-background-cover"
                    :data-src="itemName.photos[0].Url.split(1920, 1080).join(280, 180)" data-uk-img></div>
                  <div v-else class="uk-inline uk-width-1-1 img-height-medium uk-background-cover"
                    data-src="/comingsoon.jpg" data-uk-img></div>
                </div>
                ...
                <div class="uk-card-body uk-padding-small">
                  <div
                    ...
                    <div>
                      ...
                      " :uk-tooltip="`${itemName.transmission.displayValue[0]}`" class="rel-text-bold">
                        CVT
                      </div>
...
export default {
  ...
  computed: {
    vehicles(model) {
      return this.$store.state.vehicles.filter((item) => item.model.value[0] === this.model);
    },
    filteredItems: function () {
      if (this.checkedModelCat.length) {
        return this.vehicles.filter((vehicles) =>
          this.checkedModelCat.includes(vehicles.badge.displayValue[0])
        );
      } else {
        return (!this.vLodash.isEmpty(this.vehicles)) ? this.vehicles.filter((item) => item.model.value[0] === this.model) : {};
      }
    },
    itemsCount() {
    ...
    totalCount() {
      return (!this.vLodash.isEmpty(this.vehicles)) ? this.vehicles.length : {};
    },
    vehiclecat: function () {
      return (!this.vLodash.isEmpty(this.vehicles)) ? [...new Set(this.vehicles.map((i) => i.badge.displayValue[0]))] : {};
    },
  },
  filters: {
    currency(price) {
